...
- lambda_param clamped positive each forward pass
- Safer reaction denominator
"""
...
class TNRDStage(nn.Module):
    def __init__(self, num_filters=24):
        ...
        self.influences   = nn.ModuleList([RBFInfluence() for _ in range(num_filters)])

        # FIX: store as positive raw value; clamp in forward so it can never go negative
    ...
    def forward(self, u, f):
        ...
        # Keep lambda strictly positive regardless of gradient updates
        lam = torch.clamp(self.lambda_param, min=1e-4)
        ...
        # Reaction term — denominator uses 0.1 for a bit more smoothness
        reaction = lam * (u - f) / (u + 0.1)
